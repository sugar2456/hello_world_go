...
func (uq *UserQuery) QueryVideos() *VideosQuery {
	query := (&VideosClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithVideos(opts ...func(*VideosQuery)) *UserQuery {
	query := (&VideosClient{config: uq.config}).Query()
...
func (uq *UserQuery) loadVideos(ctx context.Context, query *VideosQuery, nodes []*User, init func(*User), assign func(*User, *Videos)) error {
	...
	for _, n := range neighbors {
		fk := n.user_videos
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_videos" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
